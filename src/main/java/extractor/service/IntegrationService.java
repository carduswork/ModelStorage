...
public class IntegrationService {
			...
			r.forEach((compv) -> {
				...
				processorlist.forEach((v5) -> {
					...
					tasklistinpart.forEach((taskv) -> {
						...
						// task上的propagation
						List<_exception> tskecplist = em.selectByComp(taskv.getTaskid());
						tskecplist.forEach((ecpv) -> {
							Element ecpelement = tsk.addElement("propagation");
	...
	public void GenerateIntegaraton4SYSML(String filename, String modeltype) {
		String dir = "src/main/resources/INTEGRATIONMODEL/";
		// 可配置
		try {
			Document d2 = DocumentHelper.createDocument();
